...
async function fetchData() {
    ...
    try {
        ...
        // MAIN BLCOK
        let data22 = jsonData.filtered.data;


        // VARS FOR cummulative_openinterest1 SUM
        let strikePrices_all_array = [];
        let changeinopen1_all_array = [];
        let changeinopen1_all_array_PE = [];
        ...
        console.log(changeinopen1_all_array);
        ...
        console.log('GOING INSIDE OPEN INTEREST');
        ...
        for (let i = 0; i < data22.length; i++) {
            ...
            if (typeof data22[i].PE !== 'undefined') {

                if (typeof data22[i].PE.changeinOpenInterest !== 'undefined') {
                    changeinopen1_all_array_PE.push(data22[i].PE.changeinOpenInterest);
                }
            }
                ...
                console.log('Value not found for PE, pushing 0');
                changeinopen1_all_array_PE.push(0);
            }
        ...
        console.log(changeinopen1_all_array_PE);
        ...
        console.log("Subset:", ceChangeInOpenInterestSubset);
        console.log("Sum of PE:", sum_CE_OPENINTEREST);
        ...
        console.log("PE:", sum_CE_OPENINTEREST);

        const cummulative_openinterest1 = sum_CE_OPENINTEREST - sum_PE_OPENINTEREST;



        console.log('cummulative open interest for time=' + ' ' + changeinopen1_all_array_PE[15] + ' ')
        console.log('cummulative open interest for time=' + ' ' + timestampp + ' ')
...
app.get("/", function (req, res) {
    ...
    open_in.find({ "company_name": "ABB" })
        .then((results) => {
            const timestamps = results.map((result) => result.timestamp);
            const openInterests = results.map((result) => result.cummulative_openinterest);

            console.log(Array.isArray(timestamps));
            console.log(Array.isArray(openInterests));

            res.render('index', {

                people: ['mon', 'tues', 'wed', 'thus', 'fri'],
                people1: [2500, 1149, 1144, 1124, 115],
                timestamps,
                openInterests
            });
